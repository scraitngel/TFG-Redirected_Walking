...
public class GlobalConfiguration : MonoBehaviour
{
    ...
    [Tooltip("In which dispositive the app will be running")]
    public DispChoice dispChoice;
    ...
    [HideInInspector]
    public bool readyToStart, firstTime = true;//If all users are ready for the real walking experiment
    ...
    {        
        ...
        for (var id = 0; id < redirectedAvatars.Count; id++)
        {
            ...
            if (movementController == MovementController.HMD) {
                avatarRepresentations[id].transform.position = new Vector3(rm.currPos.x, 
                                                                    avatarRepresentations[id].transform.position.y, rm.currPos.z);
                avatarRepresentations[id].transform.rotation = Quaternion.LookRotation(rm.currDir, Vector3.up);
            } else {
    ...
    public void GenerateTrackingSpace(int avatarNum, out List<Vector2> trackingSpacePoints, out List<List<Vector2>> obstaclePolygons, out List<InitialConfiguration> initialConfigurations)
    {
        ...
        if (movementController == MovementController.HMD) {
            if (dispChoice == DispChoice.Pico) {
                ...
                foreach (var point in PXR_Boundary.GetGeometry(BoundaryType.OuterBoundary/*BoundaryType.PlayArea*/)) {
                    trackingSpacePoints.Add(new Vector2(point.x, point.z));
                }
            } else {
                trackingSpacePoints = new List<Vector2>();
            }
